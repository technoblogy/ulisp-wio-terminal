/* uLisp ARM 3.3b - www.ulisp.com
   David Johnson-Davies - www.technoblogy.com - 3rd August 2020
...
#define MEMBANK
  ...
  #define CPU_ATSAMD51
    ...
    Adafruit_ST7735 tft = Adafruit_ST7735(&SPI1, 44, 45, 46);
  #endif
...
bool I2Cstart (uint8_t address, uint8_t read) {
 ...
 if (read == 0) {
   ...
   ok = (Wire1.endTransmission(true) == 0);
   Wire1.beginTransmission(address);
 }
 else Wire1.requestFrom(address, I2CCount);
 return ok;
}

bool I2Crestart (uint8_t address, uint8_t read) {
  int error = (Wire1.endTransmission(false) != 0);
  if (read == 0) Wire1.beginTransmission(address);
  else Wire1.requestFrom(address, I2CCount);
  return error ? false : true;
}

void I2Cstop (uint8_t read) {
  if (read == 0) Wire1.endTransmission(); // Check for error?
}
...
#elif defined(ARDUINO_WIO_TERMINAL)
  if (!((pin>=0 && pin<=8))) error(ANALOGREAD, invalidpin, number(pin));
#endif
}
...
#if defined(ARDUINO_NRF52840_CLUE) || defined(ARDUINO_WIO_TERMINAL)
